 ...
 * @LastEditTime: 2021-08-31 15:14:41
...
async function exifFileDateFunction(filePath, startTime) {
    ...
    try {
        ...
        exifFileDate.gps.lat = ToDigital(
            exifFileDate.gps.GPSLatitude[0],
            exifFileDate.gps.GPSLatitude[1],
            exifFileDate.gps.GPSLatitude[2]
        )
        exifFileDate.gps.lon = ToDigital(
            exifFileDate.gps.GPSLongitude[0],
            exifFileDate.gps.GPSLongitude[1],
            exifFileDate.gps.GPSLongitude[2]
        )
    ...
    if (ret.lon) {
        try {
            const resp = await getHttp(
                `https://restapi.amap.com/v3/geocode/regeo?location=${ret.lon},${ret.lat}&key=${config.GaodeKey}`
            )
            // 字符串截取
            let JsonString = JSON.parse(
                resp.substring(resp.indexOf(']') + 1, resp.length)
            )
    ...
    // ${FileRamaparsed}]oldname-
    // addInforesp = ''
    let newFileRamaparsed = `${Stime}-pe[事件-名字]-ad[${addInforesp}]-[${Make}]`
    ...
    fsPromise.rename(
        FileRamaPath,
        `${__dirname}//${'self/'}${newFileRamaparsed}${FileRamaPathExtName}`
    )
}
